...
import { IDPGame } from "../../components/prisoners-dilemma/IDPGame";
...
### Interactive Game: Try Different Strategies

Now you can experiment with different strategies yourself! Try the "cautious" strategy against the "copycat" strategy:

export const cautiousStrategy = {
  name: "Cautious",
  firstMove: "coop",
  strategy: {
...
<IDPGame
  client:idle
  myBaseStrategy={cautiousStrategy}
  oppBaseStrategy="copycat"
/>
...
### Interactive Game: Test Extortion Strategy

Try an extortion strategy against a betrayer to see how it performs:

export const extortionStrategy = {
  name: "Extortion",
  firstMove: "coop",
  strategy: {
    previousTurnIBetrayed: { coop: 40, betray: 0 },
    previousTurnICoop: { coop: 50, betray: 70 },
  },
};

<IDPGame
  client:idle
  myBaseStrategy={extortionStrategy}
  oppBaseStrategy="betray"
/>
...
<Latex formula="\mathcal{G}=\{\mathcal{N}, S, \mu\}" />

- a set of players <Latex formula="\mathcal{N}= \{P_{1}, P_{2}, P_{3}, ..., P_{N}\}" />
- a set <Latex formula="S=\{S_{1}, S_{2}, S_{3},..., S_{N}\}" />
  - the set <Latex formula="S_{i}" /> is the set of strategies for player <Latex formula="P_{i}" />
  - <Latex formula="S_{i}" /> can be anything: \{heads, tails\} or \{bet 1$, bet 5$, bet 10$\}
  - we call <Latex formula="\mathfrak{S} = \times_{i=1}^{N}S_{i}" /> the set of every situation possible
- a utility function: <Latex formula="\mu:s \in \mathfrak{S} \mapsto (g_{1}, ..., g_{N}) \in \mathbb{R}^N" />

A _zero-sum game_ is such that

<Latex formula="\forall s \in \mathfrak{S}, \sum_{i=1}^{N} \mu_{i}(s) = 0" />

A _symmetrical two-players game_ is such that

<Latex formula="\forall (s_{1}, s_{2}) \in \mathfrak{S}, \mu_{1}(s_{1}, s_{2})=\mu_{2}(s_{2}, s_{1})" />
...
Let's simplify the matrices above. The determinants are not changed by a linear operation on the columns. So let's do $\\\{C_\{1\}\\\}+\\\{C_\{2\}\\\}\mapsto \\\{C_\{2\}\\\}$ and $\\\{C_\{1\}\\\}+\\\{C_\{3\}\\\}\mapsto \\\{C_\{3\}\\\}$. We get:
